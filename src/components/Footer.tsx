  ...
  return (
    ...
    >
      <div className="relative overflow-hidden">
        ...
        <div className="transition-transform duration-300 absolute top-0 left-0 translate-y-full group-hover:translate-y-0 w-full">
          {label}
        </div>
      </div>
    </a>
...
export default function Footer() {
  return (
    <footer className="w-full bg-[#efeeec]">
      <div className="w-full px-0">
        <div className="p-2 grid relative mt-8 lg:mt-0">
          {/* Dark background with rounded corners */}
          <div className="absolute top-0 left-0 w-full p-2 flex" style={{ height: '99.9%' }}>
            <div className="w-full h-full bg-ink rounded-3xl" />
          </div>
          ...
          <div className="col-start-1 row-start-1 grid grid-cols-12 pt-14 pb-6 relative z-20 lg:py-10 px-4 md:px-7 gap-x-3 md:gap-x-5 gap-y-3 md:gap-y-7">
            ...
            <div className="flex flex-col items-start justify-start col-span-12 mb-10 lg:mb-0 lg:col-span-4 gap-y-3 md:gap-y-5">
              ...
              <div className="w-full relative">
                <input
                  ...
                  className="appearance-none bg-[#2a2a2a] rounded-full w-full text-white font-medium tracking-tight leading-none text-lg px-5 py-4 lg:text-xl lg:px-6 lg:py-5 placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-white/15"
                />
                <div className="absolute top-0 right-0 p-2">
                  <button className="size-9 lg:size-13 bg-mint text-ink text-md rounded-full flex items-center justify-center cursor-pointer transition hover:bg-white hover:rotate-90">
              ...
              <div className="flex gap-1">
                <a href="https://www.facebook.com/riseatseven" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-facebook-f" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                <a href="https://x.com/riseatseven" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-x-twitter" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                <a href="https://www.linkedin.com/company/riseatseven/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-linkedin-in" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                <a href="https://www.youtube.com/channel/UCAjOP9BgpZPTgae-QT9HGCw" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-youtube" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                <a href="https://www.tiktok.com/@riseatseven" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-tiktok" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
                <a href="https://www.instagram.com/riseatseven/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-x-2.5 rounded-xl text-xs px-2 py-1 transition hover:rounded-sm bg-white text-ink">
                  <div className="inline-flex items-center gap-x-0.5"><i className="fa-brands fa-instagram" aria-hidden="true" /></div>
                  <ArrowUpRight className="w-2.5 h-2.5" />
                </a>
              </div>
            </div>
            ...
            {/* Links columns */}
            <div className="col-span-12 md:col-span-11 lg:col-span-6 lg:col-start-6 grid grid-cols-2 sm:grid-cols-3 gap-y-10 gap-x-4 w-full">
              {/* Column 1 */}
              <div className="flex flex-col items-start gap-y-1.5 border-l border-white/20 pl-3">
                <FooterLink href="#" label="Services" />
                <FooterLink href="#" label="Work" />
                <FooterLink href="#" label="About" />
                <FooterLink href="#" label="Culture" />
                <FooterLink href="#" label="Meet The Risers" />
              </div>

              {/* Column 2 */}
              <div className="flex flex-col items-start gap-y-1.5 border-l border-white/20 pl-3">
                <FooterLink href="#" label="Testimonials" />
                <FooterLink href="#" label="Blog & Resources" />
                <FooterLink href="#" label="Webinars" />
                <FooterLink href="#" label="Careers" />
              </div>

              {/* Column 3 */}
              <div className="flex flex-col items-start gap-y-1.5 border-l border-white/20 pl-3">
                <FooterLink href="#" label="Sheffield" />
                <FooterLink href="#" label="Manchester" />
                <FooterLink href="#" label="London" />
                <FooterLink href="#" label="New York" />
                <FooterLink href="#" label="Contact" />
              </div>
            </div>

            {/* Rise at Seven SVG Logo */}
            ...
            {/* Bottom bar */}
            <div className="col-span-12 flex flex-col mt-10 items-start md:flex-row md:justify-between lg:items-center lg:mt-0 gap-y-4">
              <div className="flex gap-x-2 gap-y-2 flex-wrap items-center md:gap-3">
                {[
                  ...
                  <span key={i} className="contents">
                    <span className="text-white font-light leading-tight text-[10px] md:text-xs">{text}</span>
                    <span className="w-1 h-1 rounded-full bg-white inline-flex md:mt-0.5 last:hidden" />
                  </span>
                ))}
                {['Privacy Policy', 'Terms & conditions'].map((text, i) => (
                  <span key={`link-${i}`} className="contents">
                    <a href="#" className="text-white font-light leading-tight text-[10px] md:text-xs underline-offset-2 hover:underline">{text}</a>
                    <span className="w-1 h-1 rounded-full bg-white inline-flex md:mt-0.5 last:hidden" />
                  </span>
                ))}
              </div>

              <div className="w-full mt-1 md:w-auto md:ml-auto md:text-right lg:mt-0">
                <a href="#" className="text-white font-light leading-tight text-[10px] md:text-xs hover:underline">
                  Website MadeByShape
                </a>
              </div>
            </div>
